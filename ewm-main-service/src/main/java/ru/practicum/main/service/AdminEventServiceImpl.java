...
public class AdminEventServiceImpl implements AdminEventService {
    ...
    public Event patchAdminEvent(long eventId, AdminEvent eventNew) {
        ...
        if (event.getEventDate().isBefore(LocalDateTime.now().plusHours(1))) {
            throw new BadRequestException("It is no longer possible to change this event");
        }
        if (eventNew.getEventDate() != null) {
            if (eventNew.getEventDate().isBefore(LocalDateTime.now().plusHours(1))) {
                throw new BadRequestException("It is no longer possible to change the start date of this event");
            } else {
